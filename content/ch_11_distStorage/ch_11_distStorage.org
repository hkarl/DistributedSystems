...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Which consistency model requires that a write is instantaneously visible to all processes?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Strict consistency is impossible to implement in a distributed system. Why?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In linearizability, if o1 completes before o2 starts, what must be true?
*** Example 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Two processes each perform a write; a third process observes them in opposite orders from a fourth. Which consistency model is violated?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Causal consistency allows concurrent writes to be seen in different order on different machines. TRUE or FALSE?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
FIFO consistency guarantees order of writes from a single process. Does it also guarantee order across different processes?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
With the write-everywhere protocol, why is each site easily able to keep track of the sequence of updates?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
With N=5 replicas, N_W=3, N_R=3: does this satisfy the quorum overlap condition N_R + N_W > N?
*** Quorum examples 
****                                                              :B_columns:
     :PROPERTIES:
     :BEAMER_env: columns
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Give an everyday example of a system that provides eventual consistency.
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
A client writes at replica L1, then reads from replica L2 and sees the old value. Which client-centric guarantee is violated?
