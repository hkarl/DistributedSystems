...
\pause
     Quiz: send(buf, dest) returns immediately without waiting for the receiver. What has the OS guaranteed at this point?
*** Synchronicity of operations 
...
\pause
     Quiz: A client uses non-blocking receive and busy-polls for a reply. What is the main cost compared to blocking receive?
...
Common model: *Parallel* Random Access Machine \cite{fortune78:_pram} \cite{Karp:1988:SPA:894803}
...
\pause
     Quiz: A designer assumes all messages arrive within 100 ms. Which timing model does this describe?
*** Fault models 
...
- Reading from sockets can be blocking or non-blocking 
- Multiple sockets from which to read?
  - Non-blocking: simple, in busy loop, but wastes CPU cycles
  - Blocking: simple for blocking on *one* socket, but multiple?
- We need to tell the OS the *list of sockets* we want to block on
  - And be informed when *any one* of them becomes ready to be read 
\pause
- Supported by all relevant OSes
  - E.g., ~kqueue~ on MacOS, ~epoll~ on Linux, \dots
\pause
- We will need this ability later on for event loops in, e.g.,
  JavaScript 
...
The libzmq library is licensed under the Mozilla Public License 2.0. 
...
\pause
     Quiz: Without a neutral wire format, how many pairwise translators are needed between n programming languages?
...
- Some candidates 
  - Abstract Syntax Notation.1 (ASN.1 )  @@latex: \textemdash{} @@
    used in, e.g.,  ISO/OSI layer 6;
...
\pause
     Quiz: What does Protocol Buffers (.proto + protoc) give you that Python msgpack does NOT? (Select all that apply.)
...
#+CAPTION: Roles are not fixed: One process' server is another process' client
#+NAME: rolesNotFixed
...
- At first invocation, via a *super server daemon* 
  - ~inetd~ (deprecated), ~xinetd~, ~launchd~, ...
...
\pause
     Quiz: A server remembers the file offset between successive read calls from the same client. Is this stateful or stateless? What happens if the server crashes?
*** How to find server? 
...
\pause
     Quiz: Name one advantage and one disadvantage of the two-tier client/server architecture.
...
\pause
     Quiz: A client retransmits a bank-transfer request after a timeout. What risk arises if the server does not track processed message IDs?
