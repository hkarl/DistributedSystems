...
*** Fault assumptions 
 - Everything is completely dependable (not realistic)
\pause 
 - Processes can fail
...
\pause 
- Hence: proper terminology would be to talk about *error-tolerant*
  systems, but *fault-tolerant* is established practice  
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Fault, error, failure: An alpha particle flips a bit; a subsequent read returns the wrong value; a watchdog reboots the server unnecessarily. Which event is the fault, which the error, which the failure?
...
**** Service level objectives in clouds 

- Often: *five nines*  @@latex: \textemdash{} @@ availability of at least
  99.999%
  - Or max. 5 minutes of downtime per year 
\pause

***** Aside: SLA, SLO 

- \gls{SLO}: Defines a target value for a measureable
  quantity (e.g., availability, packet losses, percentiles of response
  time, \dots), typically averaged over certain periods 
- \gls{SLA}: Contract specifying SLOs, payments, penalties for failing
  to meet SLOs 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
A data center has 10,000 disks, each with MTTF = 500,000 hours. What is the expected time to the first disk failure?
*** Example: Disks in a data center (2) 

- Let $X_i, i=1\ldots, 10000$ be the lifetime RVs (time to failure) of
  each disk
  - We know $X \sim \exp(\frac{1}{500.000\,\mathrm{h}})$
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
A system has MTTF = 900 h and MTTR = 100 h. What is its steady-state availability?
...
- Challenge: Life gets complicated when survival probabilities are
  heavy-tailed
  - Infinite variance of RV, tail probabilities drop slower than
    exponentially 
...
- *Hot* standby
  ...
  - Necessary state is mirrored continuously (ideally, within fixed
    deadlines) 
  - Only one output is used (e.g., after voting) \pause
- *Active/active*
  - Both actual and redundant resource are up and running, process all
    requests, outputs of both systems are acted upon 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Which standby category requires the redundant resource to be powered OFF and only activated (booted) when the primary fails?
*** Abstraction level? 
...
- Example: Triple modular redundancy
  - Three subsystems propose result, voter picks majority result 
...
- Exploits that the probability of rejecting a job drops when scaling
  up both load and resources correspondingly
- For details: Queueing theory, Erlang-B model 
...
- Introduce *failure detector*  supervising  a given system  \cite{chandra96:_unrel_failur_detec_reliab_distr_system,birman12:_guide_reliab_distr_system}
 
*** Failure detector

- Has one of two opinions about a systems state
  - *Trusted*
  - *Suspect*
- Used to solve, e.g., consensus problem or atomic broadcast problem
  even if  the failure detector is imperfect itself 

*** Expectations for failure detectors \cite{chandra96:_unrel_failur_detec_reliab_distr_system}

- *Completeness*: After some time, every process that crashes is
  permanently suspected by some correct process
- *Accuracy*: After some time, a correct process is never suspected by
  any correct process 

\pause
Challenge: Distinguish a failed from a slow process! 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Failure detector property: after some time, a correct process is NEVER suspected by any other correct process. Is this accuracy or completeness?
*** Failure detector, extended  
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Naive LB: frontend forwards request to worker; worker replies directly to client. Why does this fail?
...
*** Case study: Load balancer in Kubernetes 

- Homework assignment: Figure out which load balancing approach is
  used in Kubernetes to steer traffic to virtual IP addresses of a
  service  @@latex: \textemdash{} @@ relate it to the mechanisms
  discussed above 
- Hints:
  - Each pod has its own IP address, from a cluster-internal IP
    network 
  - Pods should be able to talk to pods directly
  - Every node runs a ~kube-proxy~, programming a node's IP tables for
    DNAT'ing (destination network address translation) to the pod 
    - ~kube-proxy~ is triggered by the reconciliation loop!
  - NAT tables can load balance over multiple rewrite targets!
    - Nice summary: [[https://scalingo.com/blog/iptables][Turning IPTables into a TCP load balancer for fun and profit]]
    - But only with simple worker-picking rules, like round-robin or random

*** Case study: Load balancer in Kubernetes, reading hints 


 - Also see: [[https://learnkube.com/kubernetes-network-packets][Tracing the path of network traffic in Kubernetes]]
 - Look at ~netfilter~ and ~iptables~ for details
 - Fancier setups: eBPF in Cilium, IPVS with O(1) hashtables 
...
#+BEGIN_EXAMPLE
web.bla.com     IN A      1.2.3.101  
web.bla.com     IN A      1.2.3.102
web.bla.com     IN A      1.2.3.103  
#+END_EXAMPLE
...
*** DONE Server farms and hand-off, load balancing                 :noexport:
 ...
 - Solution 3: Redirects of URLs  @@latex: \textemdash{} @@  frontend tells client to fetch the page from another server (HTTP response status 3xx)
...
*** Case study: DNS in Kubernetes

- Every Service gets a DNS name automatically, managed by *CoreDNS*
  - Format: ~<service>.<namespace>.svc.cluster.local~
  - Resolves to the Service's ClusterIP -- not to individual pod IPs
- Pods are configured to use CoreDNS as their resolver (~/etc/resolv.conf~)
  - So ~curl http://itemstore.itemstore.svc.cluster.local~ just works inside
    the cluster
  - Try it: does it work from outside the cluster as well? E.g., when
    using ~Minikube~? 
- Recall: which load balancing mechanism from the lecture does this resemble?
  - And what are its limitations compared to the iptables/kube-proxy approach?
  - Hint: how does Kubernetes sidestep the classic DNS-LB problem?
- Further reading:  [[https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/][DNS for Services and Pods]]
...
I.e., it is (slower than) logarithmic in $n$. Not good! 
...
\cite{DBLP:journals/siamcomp/AzarBKU99} in general,
\cite{berenbrink2006:heavily_loaded_allocations} for heavy loads
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
For m >= n and n servers, checking d random servers and picking the least loaded gives expected max load O(log log n / log d) above optimal. Already much better for d=?
*** Online versions? 
...
- Even checking just two options already improves result dramatically
  (*power of two choices*)
  - We will see that again in the Linkerd homework!
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
A cache has three consistency options: (1) fixed TTL, (2) conditional GET to origin on every request, (3) server pushes updates to all caches. Which option requires the ORIGIN SERVER to take the initiative when content changes?
*** HTTP proxy 
...
- What if servers are added/removed? Causes many cache misses! 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Simple hashing: server = hash(key) % N. When a 5th server is added (N: 4 -> 5), roughly what fraction of existing cached keys must migrate?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In consistent hashing with N servers, when one new server is added, approximately what fraction of keys must migrate?
...
- Easily scales \cite{nishtala13:_scalin_memcac_faceb}, minimal configuration to add a new node 
...
*** An aside: \acf{CDN}
 ...
 - CDN \cite{Pallis:2006:CDN}: Originally, a set of coordinated caches
   ...
   - Examples: Akamai, Cloudflare, Fastly, AWS CloudFront 
 ...
 - Today: Evolved into *application-delivery networks* 
   - From static web content to HD streaming & applications
 - Also: TLS termination, DDoS mitigation 
