...
- Multicast useful
  - From client to all servers in group
    - Typically, sender does *not* receiver message
...
- Multicast: A single action for sender
  ...
  - Much cheaper than mutliple unicasts (e.g., Ethernet, wireless) 
...
- Sender or recevier(s) might crash 
...
#+ATTR_LaTeX: :height 0.9\textheight :options page=4
#+NAME: fig:mc:atomic
[[./figures/mcast_ordering.pdf]]
...
def Receive(m, seqnr):
    Insert (m, seqnr) in DeliverQeue, sorted by seqnr 
    While (m, seqnr) == (. , NextDeliver):
        Deliver(m)
        NextDeliver++
#+END_SRC
...
- Not trivial implementation problem! 
...
#+ATTR_LaTeX: :height 0.9\textheight :options page=5
#+NAME: fig:time_not_causal
[[./figures/logicalTime.pdf]]
...
#+ATTR_LaTeX: :height 0.9\textheight :options page=6
#+NAME: fig:time_not_causal
[[./figures/logicalTime.pdf]]
