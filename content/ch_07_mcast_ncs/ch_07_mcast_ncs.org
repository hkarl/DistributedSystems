...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Compared to replicated unicast, IP multicast saves bandwidth mainly on which part of the network?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
UNIFORM AGREEMENT in reliable multicast means: if one process delivers m, what must happen?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
FIFO multicast guarantees ordering per sender. What does it say about messages from DIFFERENT senders?

*** Holdback 

- FIFO example shows key technique: *holdback*
- Multicast protocol must not deliver immediately upon reception, must
  potentially delay messages
...
- Simple extension to reliable multicast
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Two senders deposit and add interest to a replicated bank account. Which ordering prevents inconsistency?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Total order multicast formally adds which property on top of reliable multicast?
...
def Receive(m, seqnr):
    ...
    While DeliverQueue.head.seqnr == NextDeliver:
        Deliver(m)
        NextDeliver++
#+END_SRC
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Why can physical timestamps NOT be used to determine causal ordering in a distributed system?
...
- Critical: receiver has delivered, then crashes 
  - It may have done something irreversible in between!
...
- A's message reaches all *other* processes in the view 
- But B and C deliver message when they were both already informed
  about A's death
...
\pause
- In fact, the notion of *logical time* developed here pervades
  distributed systems: versions in distributed databases, debugging,
  consensus on events, \dots 
...
- Message $m_A$ from A sent before message $m_C$ from C
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
If L(e) < L(e') using Lamport timestamps, what can we conclude about the relationship between e and e'?
...
#+NAME: fig:logical_time_rubberband
[[./figures/logicalTime.pdf]]
...
#+CAPTION: Events with logical time stamps
#+ATTR_LaTeX: :height 0.6\textheight :options page=4
#+NAME: fig:lamport_example_events
[[./figures/logicalTime.pdf]]
...
#+NAME: fig:lamport_example_timestamps
[[./figures/logicalTime.pdf]]
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
A process receives a message with vector timestamp vt. It sets VC := max(VC, vt) then increments VC[i]. What does VC[j] represent after this update?

*** Vector clocks and causal structure 

- Claim: Vector clocks allow us to reconstruct the entire causal
...
*** Realizing Causal Ordering: CBCAST protocol
 ...
 - \ac{CBCAST} protocol
   ...
   - UDP/IP-based, IP multicast used if possible, but usually has to
     fall back on repeated unicasts 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In CBCAST, a message from sender i with timestamp vt is held back at receiver j when which condition is violated?
...
- Experiences: \cite{Birman:1994:ISIS}
  ...
  - Typically, server groups use ISIS; clients use conventional means
    to talk to servers 
  - Evolved into VSync (dead now),  thence Derecho 

*** COMMENT Case study: \href{http://vsync.codeplex.com/}{Vsync}         
...
- Key abstraction: *Object group* 
  ...
  - Checkpointing 
\pause 
- But: *abandoned project*
 


*** Current case study: [[https://github.com/belaban/JGroups][JGroups]] 

- Members join or leave a cluster
- Members send messages to the other cluster members
- Can obtain a list of other members
- Register callbacks for message reception, membership change 
- Message semantics: specified independently of the API call, in XML
  files
  - Stacking semantics on top of each other 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In JGroups, which XML protocol element enables total-order multicast by routing all messages via a coordinator?

*** JGroups example configuration 
...
#+begin_src xml 
<config>
  <UDP mcast_port="45588"/>
  <PING/>
  <FD_ALL3/>
  <pbcast.NAKACK2 xmit_interval="500"/>
  <!-- routes all multicasts via coordinator -->
  <SEQUENCER/>                     
...
*** Fancy case study: Derecho 

- Evolution/rewrite:
  \href{https://github.com/Derecho-Project/derecho}{Derecho}
- How to speed up synchronization? Circumvent network stack
  - Built to leverage \ac{RDMA}
    ...
    - Usually, a high-performance computing / data center technique 
  - Provides *replicated objects* as core building blocks
...
- Further reading: Hybrid Logical clocks 
