...
- What should a (distributed, replicated) data store do?
  ...
  - When you ask the same question at different places, it should give
    the *same answer*
    - Slightly looser requirement than *all copies should be
      identical*
...
- But what happens when the network between replicas does not work?
  - The distributed storage is *partitioned*
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
According to the CAP theorem, which combination is impossible in a partitioned distributed system?
A) Available + Consistent   B) Available + Partition-tolerant   C) All three simultaneously   D) Consistent + Partition-tolerant
*** Pacelec 

A more detailed interpretation: 

**** Pacelec \cite{abadi12:_consis_tradeof_moder_distr_datab_system_desig}                                              :B_theorem:
     :PROPERTIES:
     :BEAMER_env: theorem
     :END:
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
PACELEC says: when there is NO partition, the trade-off is between…?
A) Availability and Durability   B) Latency and Consistency   C) Throughput and Fault-tolerance   D) Sharding and Replication
*** SQL/ACID  data bases? 
...
- We then see different tradeoffs:
...
- Only Soft state \textendash{} content might change at unpredictable times
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
What does the 'S' in BASE stand for?
A) Scalable   B) Synchronised   C) Soft state   D) Structured
*** Eventual consistency 
...
- System will eventually become consistent
  - Careful, false friend: schließlich, not eventuell 
...
- CAP: Limits on traditional data base model, typical promises
  - Made explicit by NoSQL systems
- Development: 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Which of the following is NOT a typical motivation for choosing a NoSQL database over SQL?
A) Better horizontal scalability   B) Stronger ACID guarantees   C) Schema flexibility   D) High write throughput
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Key-value stores are best described as…?
A) A table with fixed columns and rows   B) A big hash table with no enforced schema   C) A tree of nested documents   D) A graph of nodes and edges
*** Key-value: Dynamo 
...
- Highlights
  - Highly available, but sacrifices consistency (AP in the CAP
    triangle)
...
- *Partitioning & replication*: consistent hashing
  - Key space mapped to a ring; each key stored on next $N$ nodes
- *Versioning*: vector clocks track causality of updates
  - Concurrent, conflicting versions possible under partition
  - Dynamo returns all conflicting versions to client; application
    must reconcile (e.g., union of shopping-cart adds)
- *Quorum*: explicit $(N, R, W)$ parameters, requiring $R + W > N$
  for strong consistency; relax for higher availability
  - *Sloppy quorum*: if preferred nodes unavailable, write to any
    $W$ reachable nodes (\emph{hinted handoff}); re-sync on recovery
    via *Merkle trees* (detect diverged replicas efficiently)
- *Membership*: gossip-based failure detection and ring view
- No multi-key transactions (no ACID isolation)
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In Amazon Dynamo, when a preferred replica node is unavailable, writes are sent to another node and a 'hint' is stored. This mechanism is called…?
A) Merkle recovery   B) Hinted handoff   C) Sloppy quorum read   D) Vector clock merge
...
*** Document-oriented databases: MongoDB \cite{Banker:2011:MongoDB}

- *Model*: Documents are JSON-like objects (stored as BSON internally)
  - Flexible schema: no two documents in a collection need the same fields
  - Documents can be arbitrarily nested
  - Grouped into *collections* (analogous to tables, but schema-free)
- *Query language*: Rich, JSON-based query API; supports indexes,
  aggregation pipelines, geospatial queries
  - Also accepts SQL-like syntax via MongoDB Atlas SQL interface
- *CAP position*: CP by default
  - All writes go to the primary; secondaries replicate asynchronously
  - Tunable: *write concern* (how many replicas must ACK) and *read
    concern* (how fresh a read must be) allow shifting towards AP
- *Good for*: catalogs, user profiles, content management \textendash{}
  any domain where schema varies across records
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
MongoDB's default CAP position is CP. What does this mean in practice when the primary fails?
A) Reads and writes continue without interruption   B) Writes are temporarily unavailable until a new primary is elected   C) The database switches to AP mode automatically   D) Data is lost
*** Column-oriented databases: Cassandra \cite{Lakshman:2010:Cassandra}

- *Model*: Rows identified by a *partition key*; within a row, columns
  ordered by a *clustering key*
  - Wide rows: a single row can hold millions of columns
  - Inspired by both Dynamo (replication, AP) and BigTable (data model)
- *Architecture*: Masterless ring, consistent hashing \textendash{} like Dynamo,
  but for a richer data model
  - Every node is equal; no single point of failure
- *CAP position*: AP by default; *tunable consistency* per operation
  - Consistency levels: \texttt{ONE}, \texttt{QUORUM}, \texttt{ALL}
  - \texttt{QUORUM} reads + \texttt{QUORUM} writes $\Rightarrow$ strong consistency
    (analogous to $R + W > N$ in Dynamo) 
- *Good for*: time-series data, event logs, IoT telemetry \textendash{}
  append-heavy, high-throughput workloads
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In Cassandra, which condition guarantees strong (linearisable) consistency?
A) Write concern = 1   B) Read level ONE + Write level ONE   C) Read QUORUM + Write QUORUM (R + W > N)   D) ALL reads only
*** Graph databases

- *Model*: nodes (entities) and edges (relationships), both with
  arbitrary properties and labels
  - Relationships are first-class citizens  @@latex: \textemdash{} @@  stored explicitly,
    not computed via joins
- *Key property*: local traversal in $O(1)$ per hop, independent
  of total graph size (\emph{index-free adjacency})
  - Contrast: SQL join cost scales with table size
- *Query languages*: Cypher (Neo4j), Gremlin (Apache TinkerPop),
  SPARQL (RDF graphs)
- *Use cases*: social networks, fraud detection, knowledge graphs,
  recommendation engines, network topology
- *Examples*: Neo4j, Amazon Neptune, TigerGraph
- *CAP*: typically CP (Neo4j: single-master + Raft replication;
  Neptune: multi-AZ with strong consistency)
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Index-free adjacency in graph databases means…?
A) There are no indexes at all   B) Each node directly stores pointers to its neighbours, enabling O(1) per-hop traversal   C) The graph has no edges   D) Joins are computed via a global index
...
*** Example: Domain Name System (DNS) – Motivation 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
DNS uses eventual consistency. Which mechanism limits how long a resolver may cache a record?
A) Merkle tree synchronisation   B) Time-To-Live (TTL) field in resource records   C) Gossip protocol   D) Vector clock expiry
*** Dynamic DNS
...
- But terminology often sloppy here 
...
- Use a quorum! (typical $N$, $W$ relations, but makes $R$ explicit as
  well)
  - Consistency condition: $R + W > N$ for explicit read/write
    tradeoffs 
...
LinkedIn Dynamo clone http://www.project-voldemort.com/voldemort/ ,
...
*** Spanner 
 ...
 - https://research.google.com/archive/spanner.html , \cite{corbett13:_spanner}
...
- SQL semantics usually implies
  - the query language, 
  - ACID database semantics
- NoSQL emphasizes  alternative models for distributed databases and
  their consistency models 
  - Driven by key insight of CAP theorem / PACELEC interpretation
  - Query language not the point 
