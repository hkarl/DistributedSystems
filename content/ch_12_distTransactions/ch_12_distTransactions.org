...
- Previous chapter:
  - ~etcd~ provides atomic operations on individual keys
  - But what about atomicity across multiple keys or services?
    servers \pause
- Generalises:
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Which ACID property ensures a transaction's result is never rolled back once committed?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Serializability guarantees that a concurrent execution is equivalent to WHAT?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In the Lost Update example: what is the correct final balance after both transactions complete correctly?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Which ACID property is primarily the responsibility of the application programmer, not the database?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In 2PC, what message does the coordinator send to start the voting phase?
*** 2PC FSM 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In 2PC, which participant state is CRITICAL (cannot simply abort or commit on timeout)?
*** 2PC critical states 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
What extra state does 3PC add between voting and commit to avoid blocking?
*** 3PC FSM 
...
#+ATTR_LaTeX: :width 0.65\linewidth :options page=2
...
*** Why does 3PC help for node crashes/recoveries? 

(Note: No network partition assumed just yet)
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In 3PC, if a participant times out in PreCommit, what should it do?
*** Critical cases for 3PC 
...
***** Coordinator times out 
  ...
  - Coordinator times out in ~Collect~: Simple to abort 
...
***** Participant times out 
  ...
  - Participant times out in ~Voted~
    - Because coordinator has crashed
    - Contact others:
      - No other node can be in ~Commit~  (convince yourself why)
      - Some other node in ~Abort~: Abort as well
      - If *any* of the other participants  in ~PreCommit~, commit is
        safe (because coordinator *also* agreed and did send the ~PREPARE_COMMIT~!) 
      - If all other nodes are still in ~Voted~: Ok to abort 
  - Participant times out in  ~PreCommit~: broadcast to other
    nodes, tell them to commit  
...
*****                               :B_ignoreheading:
    ...
    *Note*: If any node in ~PreCommit~, no other node can be in
      ~Abort~  @@latex: \textemdash{} @@ key property of 3PC 
...
- Insight so far: As long as there are *only crashes*, 3PC is *both
  live and safe* \pause  
- To be *live*, 3PC needs to make progress even if messages are
  missing, even if the nodes are *partitioned into two groups* (or
  more)?  \pause
  - *Danger*: Still has to be safe 
- Basic technique: timeout
- Question: How to behave if *no majority* of other nodes can be
  reached? 

*** Run with partition 

- Similar scenario to Section \slideref{sec:distTrans:2pc}[s:distTrans:2pc:twocrashes]
- Difference: Coordinator crashes, *and* node A is *partitioned* off
...
- Options for B, C, E?
  - Never received ~PREPARE_COMMIT~, so they abort 
- Options for A? \pause
  - Timeout, then commit \textendash{} because *majority* of *reachable* nodes in favour
    of commit! \pause
  - Timeout, then abort \textendash{} because *majority* of *total*
    nodes not available for   commit.
...
#+NAME: fig:dT:3pc:partition_Acommit
[[./figures/3pc.pdf]]
...
*** Indistinguishability is the rub

- In the end, all distributed systems problems boil down to this core
  issue: 
  - Sometimes, a node cannot distinguish, based on local observation,
    what happened in the rest of the system
  - But it should behave differently, depending on what other nodes
    did (but it cannot know that)
\pause

- In such a case, a node has to decide between: 
  - *Safety*: I better not do anything, lest I do something wrong
  - *Liveness*: I better do something, lest I stop working 

\pause
- Your system *cannot have both*
  - If *partitions* can happen
  - We circle back to this as the FLP theorem in the next chapter 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Which commit protocol is widely used in relational databases despite its blocking risk?
...
****** Pros 
 ...
 - Fair  @@latex: \textemdash{} @@  requests are served in order 
 - Easy to implement
...
#+BEGIN_SRC python 
  ...
  def receive_request(self, request):
      request_seqNr, request_id = request
      if ( (self.state == HELD) or
           (self.state == WANTED and
                ((self.seqNumber, self.id) 
                 < (request_seqNr, request_id)))):
          self.queue.append(request_id)
      else:
          send(request_id, CLEAR) 
#+END_SRC
...
*** Example problem: Deadlock detection 
...
**** Generalization: State collection (snapshot) 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
A consistent cut requires that if a message's RECEIVE is in the cut, its SEND must also be in the cut. True or false?
...
#+BEGIN_SRC python 
  class Snapshot:
      def __init__(self, numInChannels, outChannels): 
          self.inchannels = [ [] ] * numInchannels
          self.inchannels_Done = [ False ] * numInchannels
      ...
      def on_marker (self, marker_channel):
          if not self.stateRecord:
              ...
              self.inchannel_Done[marker_channel] = True
              for oc in self.outchannels: oc.send("M") 
          else:
              self.inchannel_Done[marker_channel] = True 
#+END_SRC
...
*** So what about distributed deadlock now? 

- With distributed snapshot algorithm in place, how to solve the
  original problem?
  - E.g., distributed deadlock detection, garbage collection, etc.? 
- For deadlocks:
  - Use snapshot to collect the local wait-for graphs
  - Merge them
  - Detect cycles and act on them (e.g., break locks)
...
- In context of distributed transactions, several other algorithmic
  problems occur
  - Locking/mutex; deadlocks/garbage collection/distributed snapshot 
