...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Which API style does publish/subscribe add to the basic send/receive API?
*** Events in focus? 

- Suppose there are processes that want to learn about  some
  *events*
  - It is unknown when these events happen
  - Not every process cares about all events
- Examples for events 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Name the THREE types of event descriptions covered in this lecture.
*** Subscribe, publish, notify 

- Interested process signals its interest in particular events
  - It *subscribes*
- A process *publishes* an event
...
Overview paper: \cite{eugster03:faces_pubsub}
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Which decoupling dimension means publishers and subscribers need not know each other's identity?
*** No memory 

- Pub/sub systems, in their strict form, do not keep memory!
  - No notifications of publications that happened before a
    subscription  
\pause
- We cover systems that *do* keep memory in the next chapter: Message
  queuing 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
What happens in a strict P/S system to publications that occurred BEFORE a subscription was registered?
** Matching 
   :PROPERTIES:
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In topic-based P/S, a publication matches a subscription when their topic lists do what?
...
- Subscriptions: predicates
  - Conjunction of disjunctions of *constraints* on key/value pairs
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Content-based subscriptions match on what part of a publication?
*** Matching 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In the GROUP of centralised servers approach, what problem does flooding solve — and what problem does it NOT solve?
*** Flooding 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
CBCB stands for — and it is an example of what broader concept?
*** CBCB setup 
...
- Construct a spanning tree that comprises all $N$ servers
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In CBCB, what does each routing-table entry at a node store for each neighbour?
*** CBCB forwarding 
...
- 
   \href{https://pyzmq.readthedocs.io/en/latest/}{Structure}
...
- But: 
  ...
  - \href{https://zguide.zeromq.org/}{Longer discussion} on
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
0mq pub/sub: filtering happens WHERE — and why does this matter for scalability?
** Redis 
   :PROPERTIES:
   :CUSTOM_ID: sec:redis
   :END:
...
#+BEGIN_SRC bash
$ python3 -m venv redis-env && source redis-env/bin/activate
$ pip install redis
#+END_SRC
...
*** \href{https://redis.io/docs/manual/pubsub/}{Redis and Pub/sub} 
...
- Subscribe on multiple channels ~SUBSCRIBE~
  ...
  - Subscribed clients can only subscribe, unsubscribe, ping
    ...
    def run(self):
        for item in self.pubsub.listen():
            if item['type'] == "psubscribe":
                print(f"{self.name}: someone subscribed on channel {item['channel']}")
                continue 
                
            if item['data'] == b"KILL":
                self.pubsub.unsubscribe()
                print(f"{self.name}: unsubscribed and finished")
                break
            else:
...
#+BEGIN_SRC bash
$ python redis_ps.py
A : someone subscribed on channel test.1
A : someone subscribed on channel control
B : someone subscribed on channel test.2
B : someone subscribed on channel control
C : someone subscribed on channel test.*
C : someone subscribed on channel *.
C : someone subscribed on channel control
