...
* Message queuing semantics                                        :noexport:
...
loose coupling 
- *In time*: every looser than P/S; subscription can take place after
  publication
- *In identity*: Same 
- *In space*: Same 
...
- We use MQ to integrate legacy applications
  - Option 1: Each applications need to understand messages of each
...
- Pushed by few companies, yet claiming as open standard -- lacks
  competition 
...
- Even RPC can be recreated
  - With a
    \href{https://www.rabbitmq.com/tutorials/tutorial-six-python.html}{good
    hint at restarting servers} and issues for exactly-once 
    delivery 
...
- Presentation here mostly follows
  \href{https://kafka.apache.org/documentation/#introduction}{Kafka
...
- These are \ac{CLI} examples;
  \href{https://cwiki.apache.org/confluence/display/KAFKA/Clients}{language
  bindings exist}, of course
...
- Complex systems like Kafka not always the best choice
  - Simple semantics, simpler setup, when sophisticated semantics are
    not required  
  - E.g.,
    \href{http://tomasz.janczuk.org/2015/09/from-kafka-to-zeromq-for-log-aggregation.html}{log
    aggregation via 0mq might be superior to Kafka}  -- in this
