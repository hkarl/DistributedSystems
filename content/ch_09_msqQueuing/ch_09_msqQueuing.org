...
- Pub/sub model has some properties not always desirable
  ...
  - If no matching subscriber during publication, message is dropped
    - A feature, not a bug: simple, low overhead 
\pause
- Alternative model?
  ...
  - Entails: store a message!
    - At least until delivery, possibly longer 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Which dimension of loose coupling does message queuing improve most over pub/sub?
...
- What if message cannot be delivered?
  - E.g., too large, topic does not exist, TTL expired, message
    rejected
- Silently discard? Or predictable treatment?
  - Often: *dead-letter queue*
  - Sometimes: Differentiate further between application-level faults
...
Often used to 
- Build a complex application out of simpler ones (microservices)
- Integrate existing applications into a larger one \textendash{} so-called
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
When should a broker send an ACK to achieve exactly-once delivery without failures?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
What does 'five nines' reliability mean in practice for exactly-once delivery?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
What delivery guarantees does AMQP's transport layer support between nodes?
*** AMQP issues 

Compare, e.g., \cite{Hintjens:2008:WhatiswrongwithAMQP}
- Pushed by few companies, yet claimed as open standard \textendash{} lacks
  competition 
...
- [[https://nats.io][NATS]]
  - Quote: /The Real-Time Communication Fabric for Distributed Agents
    and Applications. NATS unifies messaging, streaming, and state
    into one real-time system  @@latex: \textemdash{} @@  connecting services, devices, and AI
    agents from cloud to edge./ 
- Simpler, modern, lightweight alternative to MQ systems
- Supports pub/sub, request/reply, streaming 
- Persistence can be added via JetStreams 
...
- Simple message queueing functionality built into Unix 
  - Should have been discussed in your operating system class already 
- In SysV, in POSIX 
...
- Reliably deliver messages
  - /without losing messages/ \Innocey 
...
*** Case study: ActiveMQ Classic, ActiveMQ Artemis 
...
- Artemis: current version (as of 2026)
...
*** ActiveMQ: \href{https://activemq.apache.org/getting-started.html}{Practical aspects}
...
- Lots of plugins
- Open source plus commercial distribution (Broadcom) 
...
*** RabbitMQ messaging model \textendash{} Example 


#+CAPTION: RabbitMQ messaging model
...
- Work queues/competing consumers \textendash{} round-robin among queue
  subscribers 
- Publish/subscribe-style; one message to many receivers
  - Needs /exchanges/: producers send to exchanges, which forward to
    multiple queues 
...
*** Distributed brokers: Clustering 
  ...
  - Ordinary (classic) queues  sit on one machine
  ...
  - Emphasises consistency and partition tolerance, *sacrifices availability* (see CAP Theorem, Section  \slideref{sec:CAPTheorem}[s:CAPTheorem]) 
...
*** Distributed brokers:  Federation
  ...
  - Emphasises availability and partition tolerance, *sacrifices
    consistency* (see CAP Theorem, Section \slideref{sec:CAPTheorem}[s:CAPTheorem]) 

*** Quorum queues 

Cmp. [[https://www.rabbitmq.com/docs/quorum-queues][Quorum queues]]

- What if some queues are long-lived, need to be highly available as
  they are critical to application architecture?
  - Maybe warrants investing additional effort?
- Aspects:
  - Publisher should ask & wait for confirmation
  - Queue only confirms to publisher after message has been replicated
    sufficiently often
    - Realised via quorum 
- Hint: Study documentation carefully! 
...
- Pushing RabbitMQ to
  \href{https://blogs.vmware.com/tanzu/rabbitmq-hits-one-million-messages-per-second-on-google-compute-engine/}{1  million events / second} possible but
  non-trivial (as of 2014) 
  - Context: Apple about 1/2 million iMessages per second (2014); WhatsApp
    about 1.6 million per second (2024)
...
- \href{https://docs.celeryq.dev/en/stable/}{Celery}: distributed
...
*** Use cases 
  - *Real-time data feeds* (e.g., track user activity on a distributed
    web server farm)  
    - Stream processing with Kafka Streams or  with additional
      framework like Apache Flink
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In Kafka, a record in a topic is delivered to how many consumers in each consumer group?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Within which scope does Kafka guarantee total message order?
...
*** Efficiency 
  ...
  - Zero copy system calls
    - From pagefile to socket: \href{https://man7.org/linux/man-pages/man2/sendfile.2.html}{sendfile}
      in Linux, e.g.
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
If a Kafka consumer writes its offset AFTER consuming a message and then crashes, what delivery semantic results?
...
- Exactly once?
  ...
  - Sometimes feasible if output of consumed messages can also store
    ...
    - But that is just pushing the problem elsewhere: application
      needs to take responsibility 
\pause
- We will see later: *guaranteeing* exactly once in general is impossible, only
  (very) high probability can be promised 
...
- Followers protect against failing leader
...
- (Some of it covered in Consensus chapter when we go in detail about Raft)
...
- In cloud scenarios,
  \href{https://hackernoon.com/benchmarking-kafka-performance-part-1-write-throughput-7c7a76ab7db1}{slightly  different numbers} (50.000 \textendash{} 300.000 on three nodes)
- [[https://developer.confluent.io/learn/kafka-performance/][Confluent numbers]] (3-node cluster on AWS): about 600 kmsg/s, 5\,ms
  p99 latency at 200 MB/s 
- Scales well when adding servers to cluster!
- But performance *highly* depends on work load (message size,
  replication level, \dots)
- Alternative: \href{https://pulsar.apache.org}{Pulsar}
  claims to outpace  Kafka 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
What is a typical single-node Kafka throughput on commodity server hardware?
*** Comparison  
...
- RabbitMQ vs. Kafka? 
  ...
  - Academic paper \cite{Dobbelaere:2017:KVR:3093742.3093908} 
  - [[https://www.cloudamqp.com/blog/when-to-use-rabbitmq-or-apache-kafka.html][Comparison from company with interest in RabbitMQ]]


*** Comparison table 

#+ATTR_LATEX: :align lp{0.38\textwidth}p{0.38\textwidth} :booktabs :center
#+CAPTION: RabbitMQ vs.\ Kafka comparison (based on \cite{Dobbelaere:2017:KVR:3093742.3093908} and \href{https://www.cloudamqp.com/blog/when-to-use-rabbitmq-or-apache-kafka.html}{CloudAMQP, 2025})
#+NAME: tab:mq:rabbitmq_kafka
|                   | RabbitMQ                                          | Kafka                                          |
|-------------------+---------------------------------------------------+------------------------------------------------|
| Primary role      | General-purpose MQ; streaming via Streams (v3.9+) | Streaming platform; MQ as secondary use case   |
| Routing           | Flexible: direct, fanout, topic, header exchanges | None built-in; use separate topics             |
| Message replay    | Yes, via Streams                                  | Yes, native                                    |
| Message priority  | Supported                                         | Not supported                                  |
| Scaling           | Vertical primarily                                | Horizontal natively (partitions, nodes)        |
| Consumer model    | Push (pull possible but discouraged)              | Pull                                           |
| Persistence       | Until ACK (classic); configurable (Streams)       | Configurable retention period                  |
| Complexity        | Lower; easier initial setup                       | Higher; more concepts (partitions, offsets, …) |
...
- Choice may depend on
  \href{https://blogs.vmware.com/tanzu/messaging-patterns-for-event-driven-microservices/}{microservice  pattern} 
...
- We are interested in having node with largest ID as leader 
...
#+CAPTION: LCR example ring: Complete process
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In Raft leader election, what happens when no candidate wins a majority of votes?
...
- Raft forms the basis of coordination, leader election in Kafka as
  well 
- Specific version: [[https://kafka.apache.org/41/operations/kraft/][KRaft]]
  - Switches from Raft's push-based to a pull-based model (better
    match with Kafka consumers' behavior)

\pause
\vfill 

- Historic note: Older Kafka versions used separate system
  (~zookeeper~) for coordination; integrating it simplified
  architecture
  - Key difference: is metadata about messages external or internal? 

**** COMMENT Notes 
- Explain difference Raft push-based, KRaft pull-based 

Claude: 

KRaft is genuine Raft at its core — same leader election (randomized timeouts, term numbers, majority vote), same safety properties (at most one leader per term, committed entries never lost), same log-based state machine replication. A student who understands the Raft section already in ch_09 understands KRaft's foundations.
The main difference is how log entries are replicated. Standard Raft is push-based: the leader sends AppendEntries RPCs to followers. KRaft is pull-based: followers fetch entries from the leader, exactly like Kafka consumers fetch from a partition. This is a deliberate architectural choice — Kafka already has a highly optimized pull-based log fetch path, so KRaft reuses it. The metadata log is literally stored as a special internal Kafka topic (@metadata).
Other adaptations:

Quorum size is small (typically 3 or 5 controller nodes), separate from the broker pool. Brokers outside the quorum observe the metadata log passively but don't vote.
Snapshots work like Raft log compaction — periodic snapshots bound log growth, so a new broker joining doesn't have to replay the full history.
Leader transfer is supported (a clean handoff), which plain Raft doesn't specify.

For your course: the relationship is a nice pedagogical bridge. KRaft is a good example of "here is a real production system that takes a textbook algorithm and adapts it to an existing architectural constraint (pull-based log)" — the invariants are preserved, the mechanism is adjusted. Since ch_09 already introduces Raft leader election and forward-references the full consensus treatment, a single sentence noting "Kafka uses a variant of Raft called KRaft for this" is enough here, with the depth deferred to wherever you cover consensus properly.
...
- Are there simple libraries to provide leader election?  \pause
  - Some, e.g.,  \href{https://github.com/sile/evel}{evel} 
- But we really should be doing this right: Integrate with Consensus
  problem 
  - etcd (Section \ref{sec:consensus_etcd})


** Consensus 

*** Broader perspective: Consensus 

- In leader election, all nodes had to agree on a single property:
  which node is the leader? 
- Broader perspective: How to agree on any, arbitrary property?


*** Expectation on consensus? 

- Everybody agrees on same value
- Only plausible values can be result of consensus
  - Values participants are interested in? 
...
In addition to leader election, consensus has been or will be used implicitly for: 
- Message ordering in atomic delivery
- Transaction commit in distributed transactions
- Cluster membership changes (everybody needs to have the same
  perspective) 
