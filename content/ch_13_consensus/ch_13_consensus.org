...
- Note: illustrating the  *terminating reliable broadcast*! 
...
- Requirements
  ...
  - *Validity*
    - If all processes start with 0, all must decide for 0
    - If all processes start with 1 *and* all messages are delivered,
      then all must decide for 1
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Which three properties must every consensus algorithm satisfy?
*** No failures at all 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In the FloodSet algorithm: why are exactly f+1 rounds needed to tolerate f crash failures?
*** FloodSet algorithm 

- Each process stores a subset W out of the valid values V 
- Initially: W is the single value that the respective process proposes 
...
- Solvable provided there are at least $n = 3f+1$ nodes available when
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
What exactly does the FLP impossibility result forbid?
*** FLP proof 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Name one practical technique that allows real protocols like Raft to make progress despite the FLP result.
...
- Two main candidates
  - PAXOS: provably correct consensus protocol 
    - E.g., used in Zookeeper (in modified form), Kafka (in old versions), Google's Chubby,  ...
  ...
  - Raft \cite{Ongaro:2014:SUC:2643634.2643666}: Simpler structure, same guarantees
    - Seems more popular nowadays
...
- Formal requirements:
  - *Safety*: Never return an incorrect result \pause
  - *Available*: As long as *any majority* of servers is operational and
    can communicate, progress is made \pause
    - Hence: Prioritizes safety over liveness 
...
*** Challenges
  ...
  - Message lost, arbitrarily delayed
    - Also implies: network partition possible 
...
- Cluster with nodes, each one having an instance of a replicated
...
- Leaders are elected and responsible for terms of varying duration
  - All other nodes become followers of that leader 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In Raft: what is a 'term', and what is its role as a logical clock?
*** Raft basics (2)
...
- Clients are expected to wait for ACK from leader and retry if
  timeout 
...
- Leader only confirms to client once it has confirmation from
  majority of servers 
  - And only then can a command actually be applied to the
    (replicated) FSM 
\pause
...
- Logs contain *committed* and *uncommitted* entries
  ...
  - *Uncommitted*: Still not confirmed  from sufficiently many other
    followers 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In Raft log replication: when is a log entry considered 'committed'?
*** Log example 

#+CAPTION: Example Raft log; colors and numbers show terms (Figure 6 from \cite{ongaro14:_in_searc_under_consen_algor_exten_version}); color and top number in boxes show term
#+ATTR_LaTeX: :width 0.6\textheight
#+ATTR_LaTeX: :caption \caption{Example Raft log (Figure 6 from \cite{ongaro14:_in_searc_under_consen_algor_exten_version}) \label{fig:raft:log}}
#+NAME: fig:raft:log
...
- Leader sends command to all followers to replicate it
...
\pause
- Commit index is piggybacked on other messages
  - So followers learn which messages have been committed and can then
    apply them to their own FSM instances as well
  - But followers do not reply to clients 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
State the Raft Log-Matching property: what two things are guaranteed if two log entries share the same (index, term)?
*** Committing on followers 

To emphasis again: All nodes apply to their FSM eventually! 
...
- Hence, followers can find out which entries are committed and should
...
- Case c, d: Leader misses entries from previous terms
  - Could happen if those nodes were leader in previous terms, but
...
How do we know there are uncommitted entries at other nodes? 
...
\pause 
- Note: we only remove uncommitted entries
  - I.e.: not yet applied to state machine, not yet acknowledged to
    client
...
- Example scenario:
  ...
  - Other leader B commits entries (majority of nodes is still up!)
  - A comes up, becomes leader in following term, and lacks the
    committed entries 
\pause 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Why can't Raft commit entries from previous terms the moment they are stored on a majority of servers?
...
- What happens when the network partitions and subgroups of nodes can
  no longer talk to each other?
- Case 1: Leaders is in a partition with majority of all nodes
  - Nothing really happens; proceeds as usual
- Case 2: Leader in minority partition
  - Nodes in majority partition no longer receiver leader heartbeats
    and will successfully elect a new leader
  - Updates to the new leader succeed; to the older leader fail (owing
    to lack of quorum)
\pause

- No split brain because it takes a majority of nodes to become leader
  / get quorum for commits  @@latex: \textemdash{} @@ this can only be
  the case in *at most one* partition 

\pause
- Homework: what happens if *no* partition has majority of nodes? 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In a 5-node Raft cluster, the network partitions into groups of 3 and 2. Which group can still make progress, and why?
*** Healing network partition 

- Partition with the newest term wins
  - Old leader in minority partition steps down and discards its
    uncommitted log entries 
- Lower terms can only have uncommitted entries; they are discarded 

*** Partitions cost liveness 

- *Consistency cost*: In presence of partitions, Raft prioritizes
  consistency over availability (cmp CAP discussion in following chapter)
  - Clients connected to (old) leader in minority partition will not get
    an ACK, will be blocked 

*** Log compaction 

How to deal with log that grows above bounds? 
...
- We saw how safety and liveness have to be balanced
...
- Use a leader elect protocol to elect a new primary
...
#+NAME: fig:consensus:pb:group_partial_partition 
[[./figures/paxos.pdf]]
...
#+NAME: fig:consensus:pb:round_numbers
[[./figures/paxos.pdf]]
...
- Let’s give unique, ordered sequence numbers to proposals
  ...
  - Uniqueness follows if, once a value $v$ for a proposal with number
...
#+CAPTION: Paxos: battling proposers
...
\href{https://raft.github.io/raft.pdf}{extended version}) 
    ...
    - More restricted than Paxos, more specified in detail,
      ...
      available   
    - E.g., prescribes that only most up-to-date server can become new
      leader
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
How many nodes are required to solve Byzantine agreement with f traitors, and why is 3f not enough?
*** Formally 
...
- Possibly natural expectation: each faulty node has to be outvoted by
  two correct nodes
  - Hence: for $f$ faulty nodes, $3f$ nodes in total suffice
  - So-called *\ac{TMR}*
...
- So: A decides 1 if and only if C decides 0 
- *Contradiction* 
...
def vote(id, initial_vote, num_proc, f):
    ...
    for k in range(f+1):
        ...
        majority_pref = most often value in preference
        multiplicity = number of occurrences of majority_pref
...
#+BEGIN_SRC python
    for k in range(f+1): 
        # odd phase: see previous slide
        # even phase: 
        if id == k:
            broadcast majority_pref
        king_majority = receive from process k
            # possibly from yourself

        if multiplicity > num_proc/2 + f:
...
|----------+---------------+-------+---------+-------+-------+-------|
| 1a       | Preferences   | 77332 | #       | 72332 | 7?332 | 72332 |
|          | Majority      |     7 | #       |     2 |     3 |     2 |
...
\pause
- Distributed ledgers, bitcoins, Nakamoto consensus 
  - Basically an implementation of Byzantine agreement in a network
    with open memberhship (so the $n>3f$ argument fails)
  - But it still only solves probabilisitic agreement with only
...
- Zookeeper, Chubby: Use Paxos (or variants thereof) to ensure consistent states
...
- \href{https://github.com/etcd-io/etcd}{etcd} (distributed, reliable
  ...
  - Already covered extensively in Section \ref{sec:consensus_etcd}
...
*** COMMENT etcd Architecture 

- Designed for small amounts of key/value data, able to fit in memory 
- With
\href{https://github.com/etcd-io/etcd/tree/master/etcdctl}{command-line client} (in particular, ~put~, ~get~, ~del~ operations) 
...
- Its impossibility in general settings is unavoidable
...
- Byznathine agreement is a significantly harder problem, requiring
  substantial overheads to solve 
