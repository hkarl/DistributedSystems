...
\pause
     Quiz: In RPC, what does the client-side stub do? Who does it pretend to be?
*** RPC by proxy 
...
\pause
     Quiz: RPC uses copy-and-restore for reference parameters. Is this always equivalent to call-by-reference? Think of f(&a, &a).
...
\pause
     Quiz: Local calls execute exactly once. Why can RPC not guarantee exactly-once semantics?
*** Source of failures
...
- Need *idempotent* function!
  - In a math sense: $\forall x: f(f(x)) = f(x)$ \pause
  - In a protocol sense: doing an action twice leaves system in *same
...
\pause
     Quiz: When a lost RPC reply triggers a retransmission, under what condition can the server just recompute the result instead of using a reply cache?
*** RPC request/reply: reply lost
...
#+CAPTION: Packet delay gets repaired
...
*** zerorpc 
An example for a simple RPC/RMI library: zerorpc
...
- Possible today's main RPC framework
- Compare
...
- Based on ProtoBuf as IDL (compare previous chapter)
  - JSON also possible 
- On top of ProtoBuf message definitions, also *service definitions* 
- Allows *streams*, with ordering guaranteed  
- Both synchronous and asynchronous
  - With optional timeouts 
- No specific termination guarantees; server / client might not agree 
- Metadata as key/value pairs
  - E.g., authentication data 
- Debugging via ~grpcdebug~ ! 
...
\pause
     Quiz: gRPC uses .proto files and protoc. What does the compiler generate from those files?
*** gRPC vs. zerorpc 

- Zerorpc: simpler, smaller learning curve, non-standard 
- gRPC: probably industry standard, ProtoBuf basis 
...
- We need to find WSDLs and binding 
...
- WebServices (as XML/SOAP/WSDL/UDDI) was once considered the saviour
  of complex IT systems  \pause
- But turned out *way* too complicated
  - Hampered by bad interoperability between different vendors' stacks
  - And browsers happened at the wrong time: Javascript could deal
    with JSON, not (easily) with XML
  - And slow, company-politics-hampered standardization process
- For history, compare 
...
\pause
     Quiz: What is the KEY architectural property that distinguishes REST from generic web services?
...
\pause
     Quiz: Which URL and method pattern is most RESTful: /books/list + /books/add, or GET /books/ + POST /books/ + DELETE /books/42/?
...
- Options? 
  ...
  - SWAGGER (https://swagger.io) – currently extremely popular 
...
*** So far: coarse-grained functional partition 
...
- Alternative: *\ac{MS}* 
  - Chop up big binary into independently runnable instances, typically
...
- Key feature: Multiple microservices can be chained together into a
  new (micro-)service \pause
- Different styles \cite{Peltz:2003:WSO}
  - *Orchestration*: Central service controls how data flows from one
    service to another, holds knowledge about MS dependencies 
    - Often (though not necessarily) by making synchronous
      request/reply calls; often thought of as *commands* 
  - *Choreography*: MSs react to events and emit *events*, without
    knowing who will pick their emitted events
    - Usually, asynchronous event transport \pause
    - We talk about *event routing* in later chapters 
...
\pause
     Quiz: What is the difference between orchestration and choreography in microservices?
*** TODO Choreography: specifying event routing

- Publisher declares *what happened* (event type + schema)
  - No knowledge of consumers
- Consumer declares *what it listens to* (subscription / filter rule)
  - Configured at the broker (Kafka topic, RabbitMQ routing key, ...)
- Formal specification: *AsyncAPI* \cite{AsyncAPI:online}
  - Machine-readable contract per service: publishes / subscribes
  - Analogous role to OpenAPI for REST
  - Tooling: docs, validation, code generation
- Schema compatibility enforced by a *Schema Registry*
  - Ensures producers and consumers agree on event shape over time


*** Example pattern: API Gateway 

- Suppose your application comprises many microservices
  - \dots with different APIs, strutcures, level of detail
  - \dots servers different types of clients (e.g., desktop vs. mobile,
    local vs. WAN)
- Hard for clients to make calls to all relevant microservices
- Idea: Hide your actual microservices behind a common facade: the *API
  gateway service*
  - Proxies calls to other MSes, composes results 
- Compare https://microservices.io/patterns/apigateway.html



*** Consequence
- Full process flow is *implicit* -- emerges from all subscriptions together
- Advantage: loose coupling, independent deployability
- Disadvantage: hard to see the whole picture
  - Mitigated by distributed tracing (OpenTelemetry, Jaeger)
...
- We need more computational power at middle service
  ...
  - *Scale out*: Run multiple instance
    - Challenge: *Splitting* and *merging*, *discovery* 
...
\pause
     Quiz: A slow microservice is struggling. What does 'scale out' mean, and what new challenge does it introduce?
*** Discovering instances 

- Suppose microservices are scaled out/in
  - Changes set of running instances
  - Changes set of IPs under which
    an instance can be found; typically dynamically assigned 
- How to find service instances?
- Typical pattern: Service-side discovery via a service registry and
  service router 
  - Instances register with registry upon startup
  - Service router polls registry (or pushed)
    - Might nicely combine with additional patterns like Circuit
      Breaker 

*** Service registry, service router 

#+caption: Putting a service router between microservices, assisted by a service registry (green: registration path, black: data paht)
#+attr_latex: :width 0.95\textwidth :height 0.6\textheight :options keepaspectratio,page=4
#+NAME: fig:service_router_registry
[[./figures/orch_choreo.pdf]]
...
- How to deal with crashing services? 
  - Buzzword: Automatic restart, Circuit breaker, Database per
    Service, CAP theorem (see later chapter)
...
\pause
     Quiz: Ten microservices share one central SQL database. What problem does this create, and what is the recommended remedy?
*** CQRS 

- Observation: Simple model of CRUD data stores insufficient for
  complex scenarios 
  - Read access: various aggregations, filters, ...
...
Platforms 
- Don't build products, build platforms (out of / for microservices)
  - Cmp. e.g. https://courses.cs.washington.edu/courses/cse452/23wi/papers/yegge-platform-rant.html,
    \cite{vinoski08:_restf_web_servic_devel_check}


** Case study: Reactive                                            :noexport:
