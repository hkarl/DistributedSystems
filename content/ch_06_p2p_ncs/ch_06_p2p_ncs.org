...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
What is the main weakness of memcached's approach to mapping keys to servers?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In a DHT, what kind of identifier do both nodes AND stored objects share?
*** API for DHT 
...
- How do we build a system that routes on keys, not (IP) addresses?
- Next sections: work upwards from a basic idea about distributed hash
  tables to functioning P2P systems 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
What is the average number of forwarding hops in the strawman ring DHT with N nodes?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
What technique does Chord use to reduce search hops below O(N)?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
For a node with GUID n using m-bit IDs, what GUID does the i-th finger entry point to?
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
In Plaxton routing, what determines which ROW of the routing table is used for forwarding?
...
- In row $r$ and column $c$ of node $v$'s routing table, we store information about nodes whose
  GUID 
...
3. If not, find first row $r$  where $v$ and $n$ differ
4. Use the entry in row $r$, column is the $(r+1)$st digit of $n$
5. If none, backtrack to row with fewer coinciding digits

\pause

If no entry at all found, forward to some node in leaf set 
\pause
...
- Properties
  - Number of nodes visited for search request $\log_b(N)$
    - $N$ = #nodes in overlay graph, $\log_2 b$ is number of bits per group
...
- Details on mechanisms like host integration, host failure, ... 
...
- Options 
  ...
  - "Well-known" web page / bootstrap servers 
  - Broadcast
  - Expanding ring multicast
\pause
- None of them are ideal; usually, bootstrapping via well-known
  servers is used 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
What is the main practical problem with broadcast-based bootstrapping in large P2P networks?
...
- Network dynamics (addition/deletion of objects/nodes)
  - IP routing tables are updated asynchronously on a best-efforts
    basis with time constants on the order of minutes, up to an  hour (rare BGP issues)
...
- At some point, considered a vital threat to entire entertainment
  industry
  - And had a lion's share of Internet traffic (late 1990s to about  mid/late 2000s)
...
*** Early examples & generations  
 ...
 - “Generations” afterwards
   ...
   - Third: P2P middleware, e.g., Pastry, Tapestry, CAN, Chord, Kademlia, …  (2001-…) 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
Which generation of P2P systems first achieved a bounded (O(log N)) number of hops to any target?
*** Data rate asymmetry 

- Apart from robustness, second reason for file sharing systems:
  swarming!
  - Compare similar ideas of convergecasting 
...
- Justified by typical asymmetric data rates in residential setups
  - \ac{ADSL}: larger download than upload rate (but gets less and
    less relevant even for residential users)


*** Practical challenge: NAT traversal 

- In practice, not every node has a publicly visible IP address, but
  sits behind a Network Address Translation (NAT) device
- Techniques like hole punching, STUN, TURN, \dots necessary
- But this is covered in the Computer Networks course 
...
- Emphasises splitting of big files into pieces
...
- P2P aspect: value in key/value sense is list of peers seeding that
  torrent 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
What piece-selection strategy does BitTorrent use to prevent pieces from becoming unavailable?
*** Mainline DHT  
...
- Notion of “distance” between nodes different from Pastry’s
  ...
  - Symmetry entails: routes can be learned while forwarding 
...
**** PINGO!                                                       :B_alertblock:
     ...
     :END:
\pause
What key property does the XOR metric in Kademlia have that the Chord clockwise metric does NOT?
...
- Modern use case, e.g., peer discovery in decentralised networks
  (e.g., Ethereum, IPFS)
  - Cmp. ~libp2p~ 
